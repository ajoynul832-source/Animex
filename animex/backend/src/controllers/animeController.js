...
/*
Normalize Jikan → frontend format
*/
function normalizeAnime(a) {
  ...
  return {
    id: a.mal_id || a.id,
    name: a.title || a.name || 'Unknown Anime',
    poster:
    ...
    episodes: {
      sub: typeof a.episodes === 'number' ? a.episodes : null,
      dub: null
    }
  };
}
...
exports.getHome = async (req, res) => {
  try {
    ...
    const topData = (top?.data || []).map(normalizeAnime);
    const seasonData = (season?.data || []).map(normalizeAnime);
...
exports.getSchedule = async (req, res) => {
  try {
    ...
    res.json({
      data: {
        scheduledAnimes: (data?.data || []).map(normalizeAnime)
      }
    });
  } catch (err) {
    res.status(502).json({ error: err.message });
...
exports.getSources = async (req, res) => {
  res.json({
    data: {
      sources: [],
      note: 'Streaming sources removed in Jikan backend'
    }
  });
};
