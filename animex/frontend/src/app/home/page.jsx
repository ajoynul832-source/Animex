...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    userApi
      .getHistory()
      .then((d) => setHistory((d.history || []).slice(0, 12)))
      .catch(() => {});
  }, [user]);

  /*
    Jikan backend mapping
    old HiAnime fields removed
  */

  const spotlight = home?.spotlightAnimes || [];
  const trending = home?.trendingAnimes || [];

  const latest = home?.topAiringAnimes || [];
  const airing = home?.topAiringAnimes || [];

  const popular = home?.trendingAnimes || [];
  const favorite = home?.trendingAnimes || [];

  const completed = home?.topAiringAnimes || [];
