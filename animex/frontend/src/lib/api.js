...
async function req(path, opts = {}) {
  const token = typeof window !== 'undefined' ? localStorage.getItem('animex_token') : null;
...
const post   = (p, b, o) => req(p, { method: 'POST',   body: JSON.stringify(b), ...o });
const put    = (p, b, o) => req(p, { method: 'PUT',    body: JSON.stringify(b), ...o });
const del    = (p, o)    => req(p, { method: 'DELETE', ...o });

export const animeApi = {
  getHome:        ()          => req('/anime/home'),
  getSchedule:    (date)      => req(`/anime/schedule${date ? `?date=${date}` : ''}`),
  getInfo:        (id)        => req(`/anime/info/${id}`),
  getEpisodes:    (id)        => req(`/anime/episodes/${id}`),
  getSources:     (epId, srv='hd-1', cat='sub') => req(`/anime/sources?episodeId=${epId}&server=${encodeURIComponent(srv)}&category=${cat}`),
  ...
  getByGenre:     (g, p=1)    => req(`/anime/genre/${encodeURIComponent(g)}?page=${p}`),
  getAzList:      (l='all', p=1) => req(`/anime/az-list?letter=${l}&page=${p}`),
  getStats:       (pid)       => req(`/anime/stats/${pid}`),
  incrementView:  (pid, aid)  => post(`/anime/stats/${pid}/view`, { animeId: aid }),
  setReaction:    (pid, r, aid) => post(`/anime/stats/${pid}/react`, { reaction: r, animeId: aid }),
};

export const searchApi = {
  search:         (kw, p=1, filters={}) => req(`/search?keyword=${encodeURIComponent(kw)}&page=${p}&${new URLSearchParams(filters)}`),
  getSuggestions: (kw) => req(`/search/suggestions?keyword=${encodeURIComponent(kw)}`),
};
